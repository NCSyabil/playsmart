...
//                 } catch (error) {
//                     console.warn(`  Could not save sessionStorage for current origin ${currentOrigin}:`, error);
//                 }
//             }
//         }
...
//             } catch (error) {
//                 console.warn(`  Could not save sessionStorage for origin ${origin}:`, error);
...
export async function saveSession(page: Page, sessionName: string, additionalUrls: string[] = []): Promise<void> {
    ...
    try {
        ...
        if (limitedOrigins.length < validOrigins.length) {
            console.log(`  Limited to ${limitedOrigins.length} origins for performance`);
        }
        ...
        if (currentUrl) {
            ...
            if (currentOrigin && !page.isClosed()) {
                try {
                    ...
                    console.warn(`  Could not save sessionStorage for current origin:`, error);
                }
            }
        }
        ...
        for (const origin of otherOrigins) {
            try {
                ...
                if (page.isClosed()) {
                    console.warn(`  Page closed during navigation to ${origin}`);
                    break;
                }
                ...
                console.warn(`  Could not save sessionStorage for origin ${origin}:`, error.message);
                state.sessionByOrigin[origin] = {};
                ...
                if (failureCount >= 3) {
                    console.warn('  Multiple origin failures, stopping further navigation');
                    break;
                }
            }
        }
        ...
        if (additionalUrls.length > 0) {
            for (const url of additionalUrls) {
                try {
                    ...
                    console.warn(`  Could not save sessionStorage for ${url}:`, error);
                }
            }
        }
  ...
  try {
    ...
    for (const origin of origins) {
      try {
        ...
        console.warn(`  Could not restore sessionStorage for origin ${origin}:`, error);
      }
    }
    ...
    try {
        ...
        if (storageState?.cookies && Array.isArray(storageState.cookies)) {
            try {
                ...
                console.warn('  Failed to add cookies:', error);
            }
        }
        ...
        for (const origin of discoveredOrigins) {
            try {
                ...
                console.warn(`  Could not restore storage for origin ${origin}:`, error);
            }
        }
...
export function deleteSession(filePath: string): void {
  try {
    if (fs.existsSync(filePath)) {
      ...
      console.log(`  Session file deleted: ${filePath}`);
    }
  } catch (error) {
