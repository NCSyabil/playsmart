...
export async function waitForPageToLoad(
  ...
  try {
    ...
    console.warn(" DOMContentLoaded not detected within timeout");
  }
  ...
  try {
    ...
    console.warn(" Page load event not triggered within timeout");
  }
  ...
  try {
    ...
    console.warn(" requestIdleCallback failed or delayed");
  }
}
...
export async function attachLog(
  ...
  if (isCucumberRunner()) {
    ...
    if (world?.attach) {
      ...
      console.warn(" No World.attach() available in Cucumber context");
    }
    ...
    console.warn(" attachLog: Unknown runner type");
  }
}
...
export async function waitForUrl(
  ...
  async function doWaitForUrl() {
    try {
      ...
      throw new Error(` waitForUrl failed: ${error.message}`);
    }
    await waitForPageToLoad(page);
    await processScreenshot(
      page,
  ...
  try {
    if (isCucumberRunner()) {
      ...
      if (world && typeof world.attach === "function") {
        ...
        console.warn(` No Cucumber World context. Screenshot not attached.`);
      }
    } else if (isPlaywrightRunner()) {
      ...
      console.warn(` Unknown runner context. Screenshot not attached.`);
    }
  } catch (error) {
    console.warn(` Error attaching screenshot: ${error.message}`);
  }
}
